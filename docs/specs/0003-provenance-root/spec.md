...
> **Re-seal provenance (SOL-126).** Transcribed verbatim from the SOL-112 ticket
> description (the sealed `/specify` step-7 output), per the SOL-107 re-seal
> pattern. Linear `<issue>` connector tags are flattened to plain `SOL-NNN`
> identifiers; no content is added or altered. The companion four-hat review is
> at `four-hat-review.md` and the decomposition at `decomposition.md`.

**Linear:** SOL-112 (`[SOL] Backlog`, team Solo Claude Stack). This Linear workspace is the meta-project tracking surface.
**Repo:** `OndraMasek/Solo-Vibing` (target).
