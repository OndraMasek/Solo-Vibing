...
- `.claude/skills/specify/SKILL.md` — step 3 amendments per AC-6 (pyramid populator from D3.2 catalog cached as a const block in the skill; per-test `[tag]` resolution rules; `artifact_path` drafting per D3.3 per strategy; `artifact_type` recording for capability-cluster reading from `docs/templates/capability-artifact-types.md`); step 7 evaluates the five `spec.*` gates from D3.4 (`spec.strategy-annotation` clears step-1 annotation, `spec.pyramid-shape`, `spec.ac-coverage`, `spec.strategy-evidence`, `spec.provenance`).
- `.claude/skills/plan/SKILL.md` — evaluate the `plan.*` gates from D3.4 (`plan.provenance`, `plan.decomposition-shape`, `plan.child-inheritance` which fires §hybrid-without-child-overrides if any hybrid-parent child lacks an explicit strategy); D3.1 decomposition-override findings flow through the existing incorporate/defer/reject critique pattern and write to `decomposition.md` under each child's block as a `Strategy:` field with override-rationale.
